...
app.post("/provider_details", verifyToken, async (req, res) => {
  ...
  expirationTime.setSeconds(expirationTime.getSeconds() + 18 * 60 * 60 * 1000);
...
app.get("/provider_history_curr", verifyToken, async (req, res) => {
  ...
  try {
    const result = await db.query(
      "SELECT * FROM foodDetails WHERE provider_id = $1",
      [providerId]
    );
    if (result.rows.length === 0) {
...
app.get("/provider_history_prev", verifyToken, async (req, res) => {
  ...
  try {
    const result = await db.query(
      "SELECT * FROM foodDetailsHistory WHERE provider_id = $1",
      [providerId]
    );
...
app.get("/provider_details_dashboard", verifyToken, async (req, res) => {
  // console.log("You are here");
  const providerId = req.userId;
  // console.log("provider : " + providerId);
  try {
    const result = await db.query("SELECT * FROM provider WHERE id = $1", [
      providerId,
    ]);
    if (result.rows.length === 0) {
      return res.status(200).json([]);
    ...
    if (result.rows.length > 0) {
      const insertPromises = result.rows.map(async (row) => {
        ...
        try {
          ...
          await db.query("DELETE FROM foodDetails WHERE food_id = $1", [
            food_id,
          ]);
